const CYCLE = 130; // should not be the current cycle
const N = 6; // Number of async calls that can be made in parallel
const apiURL = 'https://api5.tzscan.io';
const api2URL = 'https://api.tzbeta.net:8080';
const testMode = false;
const LIMIT_DONATIONS = false; // Only count 1 tez donations
...
async function init() {
	...
	println("\nDonators: ");
	println("Donator | Identity | Parent | Genesis distance | Genesis allocation");
	var m = donationAddresses.length;
	if (testMode & donationAddresses.length > 100)
		m = 100;
	...
	println("Total balance on genesis distance 1: " + genesisBalance1[1] + " mutez");
	println("Total balance on genesis distance 0 + 1: " + (genesisBalance1[0] + genesisBalance1[1]) + " mutez\n");
	
	// delegators
	println("\Delegators for cycle " + CYCLE + " to a donator: (" + delegators.length + ")");
	println("Delegatee | Delegator | Identity | Parent | Genesis distance | Genesis allocation");
	m = delegators.length;
	if (testMode & delegators.length > 100)
		m = 100;
	...
	println("Total balance on genesis distance 1: " + genesisBalance2[1] + " mutez");
	println("Total balance on genesis distance 0 + 1: " + (genesisBalance2[0] + genesisBalance2[1]) + " mutez");
	var totalXtz = genesisBalance1[0] + genesisBalance1[1] + genesisBalance2[0] + genesisBalance2[1];
	var totalWallets = levelB[0].length + levelB[1].length + levelD[0].length + levelD[1].length;
	println("\nTotal genesis balance for donators + delegators, level 0 + 1: "  + totalXtz
	+ " mutez (" + totalWallets + ")");
	log("Done!");
	
	// Create summary
	var summary = '##################################################  START SUMMARY  ##################################################\n\n'
	+ 'The Commonwealth Fundraiser (CF) baker received donations from ' + uniqueDonationAddresses
	...
	+ ' tez (XTZ) directly from the fundraiser allocation. Through\nthe Tezos delegation feature these bakers serve as an elected '
	+ 'representative on behalf of a much larger group of participants, that have a fundraiser allocation of '
	+ mu2tez(totalXtz) + ' tez from\n' + totalWallets + ' fundraiser wallets. Combined, this would equal ' + (Math.round((10000 * mu2tezInt(totalXtz)) / TOTAL_GENESIS_BALANCE) / 100)
	+ '% of the total genesis balance (TF and DLS excluded) If we disregard\nthe HW assumption (see details below), the previous numbers would be ' + mu2tez(genesisBalance1[0]) + ' tez, '
	+ mu2tez(genesisBalance1[0] + genesisBalance2[0]) + ' tez, ' + (levelB[0].length + levelD[0].length) + ' wallets and '
	+ (Math.round((10000 * mu2tezInt(genesisBalance1[0] + genesisBalance2[0])) / TOTAL_GENESIS_BALANCE) / 100) + '%.\n\n'

	+'This data was collected from the Tezos blockchain at '
+ new Date().toLocaleString() + ' by looking at the donations to the CF\naddress' 
	+ 'and the active delegations at cycle ' + CYCLE + '. The donors fundraiser allocations were tracked and in case someone \n'
...
function getDonationAddresses(address, accounts = [], page = 0) {
	...
	return new Promise(resolve => {
		$.ajax({
		type: "GET",
		url: apiURL + "/v3/operations/" + address + "?type=Transaction&p=" + page + "&number=50",
		success: function(d){
			for (var i = 0; i < d.length; i++) {
				var amountOk = true;
				if (LIMIT_DONATIONS)
					amountOk = d[i].type.operations[0].amount ===  1000000;
				if (d[i].type.source.tz !== address && amountOk && 
			...
			if (d.length < 50)
				...
				resolve(getDonationAddresses(address, accounts, ++page));
		}});
	});
}
...
function getDelegators(address, accounts = [], page = 0) {
	return new Promise(resolve => {
		$.ajax({
		type: "GET",
		url: apiURL + "/v1/rewards_split_fast/" + address + "?cycle=" + (CYCLE + 5) + "&p=" + page + "&number=50",
		success: function(d){
			for (var i = 0; i < d.length; i++) {
				if (d[i][1] !== '0' && d[i][1] !== 0)
					accounts.push(d[i][0].tz);
			}
			if (d.length < 50)
				resolve(accounts);
			else
				resolve(getDelegators(address, accounts, ++page));
		}});
	});
}
async function getDelegatorData(KT, baker) {
	var delegate = {"KT": KT, "tz": "", "parent": "", "genesisDistance": "1+", "balance": 0};
	if (baker)
		delegate.baker = baker;
		...
		$.ajax({
			...
			success: function(d){
				...
				if (transactionsCount > 0) {
					$.ajax({
						type: "GET",
						url: apiURL + "/v3/operations/" + address + "?type=Transaction&p=" + --transactionsCount + "&number=1",
						success: function(d){
				...
				} else { // For special cases with external originator (e.g. tz1PNg922K3xkkNp8nSi8WrZNZGCQZYCQD5b)
					$.ajax({
						type: "GET",
						url: apiURL + "/v3/number_operations/" + address + "?type=Origination",
						success: function(d){
							var originationCount = d[0];
							$.ajax({
								type: "GET",
								url: apiURL + "/v3/operations/" + address + "?type=Origination&p=" + --originationCount + "&number=1",
								success: function(d){
									var parent = d[0].type.source.tz;
									if (parent.substring(0,2) === "KT") {
										resolve(getManager(parent));
...
function removeDuplicates(level, dep = []) {
	...
	if (dep.length > 0) { // If a genesis allocation is accounted for a baker, it should not also be accounted for a delegate
...
function existInBakerAllocation(tz, dep) {
	if (dep[0].findIndex(a => a.tz === tz) !== -1)
		return true;
	if (dep[1].findIndex(a => a.parent === tz) !== -1)
		return true;
	return false;
}
function getActivation(tz) {
	return new Promise(resolve => {
		...
		success: function(d){
			balance = 0;
			if (d.length)
				balance = d[0].type.operations[0].balance;
			resolve({"done": d.length, "balance": balance});
		}});
	});
}
function split(data, m) { // Print out and return arrays with 0 & 1 genesis distances
	var level = [[], []];
	for (var i = 0; i < m; i++) {
		var baker = "";
		if (typeof data[i].baker !== 'undefined')
			baker = data[i].baker + "\t";
		println(baker + data[i].KT + "\t" + data[i].tz + "\t" + data[i].parent + "\t" + data[i].genesisDistance + "\t" + data[i].balance);
...
function getGenesisBalance(level) { //Print out and return total genesis balances
	var genesisBalance = [0, 0];
	println("\n0-level: (" + level[0].length + ")");
	println("Address | Identity | Genesis allocation");
	for (var i = 0; i < level[0].length; i++) {
		println(level[0][i].KT + "\t" + level[0][i].tz + "\t" + level[0][i].balance);
	...
	for (var i = 0; i < level[1].length; i++) {
		println(level[1][i].KT + "\t" + level[1][i].tz + "\t" + level[1][i].parent + "\t" + level[1][i].balance);
